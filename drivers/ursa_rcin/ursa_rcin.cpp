...
volatile uint32_t *RC_IN::pcm_reg;
volatile uint32_t *RC_IN::clk_reg;
volatile uint32_t *RC_IN::dma_reg;
...
int RC_IN::rc_in_init()
{
	...
	circle_buffer = new Memory_table(RCIN_RPI_BUFFER_LENGTH * 2, 2);
    con_blocks = new Memory_table(RCIN_RPI_BUFFER_LENGTH * 113, 2);
...
void *RC_IN::map_peripheral(uint32_t base, uint32_t len)
{
...
void RC_IN::init_dma_cb(dma_cb_t **cbp, uint32_t mode, uint32_t source, uint32_t dest, uint32_t length, uint32_t stride, uint32_t next_cb)
{
    (*cbp)->info = mode;
    (*cbp)->src = source;
    (*cbp)->dst = dest;
...
void RC_IN::termination_handler(int signum)
{
    stop_dma();
}
...
void RC_IN::init_PCM()
{
    ...
    usleep(100);
    clk_reg[RCIN_RPI_PCMCLK_DIV] = 0x5A000000 | ((50000/RC_SAMPLE_FREQ)<<12);   // Set pcm div. If we need to configure DMA frequency.
    usleep(100);
...
void RC_IN::set_sigaction()
{
    for (int i = 0; i < NSIG; i++) {
        ...
        if (sa_old.sa_handler == nullptr) {
            sa.sa_handler = RC_IN::termination_handler;
